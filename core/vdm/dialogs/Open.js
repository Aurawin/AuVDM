coDialogs.App.Components.Open = {
  Version        : new Version(2014,10,23,13),
  Title          : new Title("Aurawin Open Dialog","Open Dialog"),
  ...
  Header         : coAppKit.Dependencies.Create(coDialogs.App,'/core/vdm/dialogs/Open.js',coAppKit.PreLoaded),
  debugToConsole : true,
  Screen         : null,
  ChangeDelay    : 250,
  Create:function(){
    ...
    sc.tmrValueChange.onExecute=function(tmr){
      ...
      sc.Nav.Confirm.setButtonOKCaption(
        (sc.ListView.isFileListed(sName)==true) ? coLang.Table.Labels.Open : coLang.Table.Labels.Select
      );
    };
    ...
    //sc.Tree.clearContainerClass("OpenDialogTreeView");
    ...
    //lv.clearContainerClass("OpenDialogListView");
    ...
    sc.ListView.onItemSelected=function(itm){
      ...
      if (itm){
        ...
        sc.Nav.Confirm.setButtonOKCaption(
          ((sFileExt==sExt) || (sExt=="") )? coLang.Table.Labels.Open : coLang.Table.Labels.Select
        );
    ...
    sc.Nav.Confirm=sc.Nav.Items.addItem(
      ...
      [
        function(navItem){  // Select
          ...
          if (  (sc.TopBar.Value.getTextLength()==0) || (sc.Folder==null) || (sc.File==null) ) return false;
          if (sc.onExecuted) sc.onExecuted(sc);
          sc.Conseal();
        },
