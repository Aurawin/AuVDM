var coDateTime={
  Version        : new Version(2014,3,9,46),
  Title          : new Title("Core Object Date Time","Core Object Date Time Utility"),
  ...
  Header         : coAppKit.Dependencies.Create(null,'/core/app/coDateTime.js',coAppKit.PreLoaded),
  ...
  Now : function(){
    ...
    if (d < 0) {
      return coMath.Trunc(d) - Math.abs(coMath.Frac(t));
    } else {
      return coMath.Trunc(d) + Math.abs(coMath.Frac(t));
    };
  },
  Date : function(iYear,iMonth,iDay){
    ...
    dt.toString=function(iFormat){
      if (iFormat==undefined) iFormat=coDateTime.formatShort;
      var dt=this;
      ...
      var sOut="";
      switch (iFormat){
        case (coDateTime.formatShort) : sOut=coLang.Table.Date.Format.Short; break;
        case (coDateTime.formatLong) : sOut=coLang.Table.Date.Format.Long; break;
      };
  ...
  Time : function(iHour,iMinute,iSecond,iMilliSecond){
    ...
    tm.toString=function(iFormat){
      ...
      var sOut="";
      switch (iFormat){
        case (coDateTime.formatShort) : sOut=coLang.Table.Time.Format.Short; break;
        case (coDateTime.formatLong) : sOut=coLang.Table.Time.Format.Long; break;
      };
  ...
  decodeTime : function(dtValue){
    var iVal = Math.floor(Math.abs(coMath.Frac(dtValue)) * coDateTime.MSecsPerDay);
    ...
    return coDateTime.Time(iHour,iMinute,iSecond,iMilliSecond);
  },
  decodeDate : function(dtValue){
    var iYear=0,iMonth=0,iDay=0, iVal=0;
    if (dtValue <=-coDateTime.DateDelta){
      return coDateTime.Date();
    } else {
      var iVal=coMath.Pred((coMath.Trunc(dtValue) + 693900) << 2);
      var iYear=coMath.Div(iVal,146097);
      iVal=iVal - (146097 * iYear);
      iDay=iVal >> 2;
      iVal=coMath.Div((iDay << 2) + 3,1461);
      iDay= ( (iDay << 2) + 7 - (1461*iVal)) >> 2;
      iMonth=coMath.Div( ( (5 * iDay) -3), 153);
      iDay= coMath.Div( ( (5 * iDay) +2 - (153*iMonth) ) , 5);
      ...
      return coDateTime.Date(iYear,iMonth,iDay);
    }
  },
  decodeDateTime : function(dtValue){
    dt=new Object();

    dt.Date=coDateTime.decodeDate(dtValue);
    dt.Time=coDateTime.decodeTime(dtValue);
    dt.toString=function(iFormat){
      if (iFormat==undefined) iFormat=coDateTime.formatShort;
      var dt=this;
      ...
      var sOut="";
      switch (iFormat){
        case (coDateTime.formatShort) : sOut=coLang.Table.DateTime.Format.Short; break;
        case (coDateTime.formatLong) : sOut=coLang.Table.DateTime.Format.Long; break;
      };
  ...
  decodeDateWeek:function(dtValue){
    ...
    var iDOY=coMath.Trunc(dtValue-dtYs)+1;
  ...
  encodeDate:function(iYear,iMonth,iDay){
    if (
      ...
      var c=coMath.Div(iYear,100);
      var ya=iYear - 100*c;
      var D=((146097*c) >> 2) + ((1461*ya) >>2) + coMath.Div(((153*iMonth)+2),5) + iDay;
      D-=693900;
      return D;
    } else {
      return null;
  ...
  secondsBetween:function(dtNow,dtThen){
    return coMath.Trunc( (Math.abs(this.dateTimeDiff(dtNow,dtThen))+this.DateTimeEpsilon)*this.SecsPerDay);
  },
  minutesBetween:function(dtNow,dtThen){
    return coMath.Trunc( (Math.abs(this.dateTimeDiff(dtNow,dtThen))+this.DateTimeEpsilon)*this.MinutesPerDay);
  },
  hoursBetween:function(dtNow,dtThen){
    return coMath.Trunc( (Math.abs(this.dateTimeDiff(dtNow,dtThen))+this.DateTimeEpsilon)*this.HoursPerDay);
  },
  daysBetween:function(dtNow,dtThen){
    return coMath.Trunc( Math.abs(this.dateTimeDiff(dtNow,dtThen))+this.DateTimeEpsilon);
  },
  dayOfWeek:function(dtValue){
    iResult= 1 + ((coMath.Trunc(dtValue) - 1) % 7);
    If (iResult<=0)
      iResult+=7;
    return iResult;
  },
