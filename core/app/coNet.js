...
var coNet = {
  ...
  SMTP_FOOTER                              : "\r\n.\r\n",
  FIELD_SEPARATOR                          : "=",
  RestrictedHeaders                        : coList.StringArray(),
  ...
  init : function (Server,NameSpace,Creds,onNetworkFail){
    coNet.RestrictedHeaders.Add("Content-Length");
    coNet.RestrictedHeaders.Add("Host");
    coNet.RestrictedHeaders.Add("Connection");
    var _net=new Object();
    _net.Class="Net";
    _net.Shutdown=false;
    _net.Failed=false;
    ...
    if (_net.WebSockets==false) coNet.FIELD_SEPARATOR = ": ";
    ...
    _net.Timers=coNet.createTimers(_net);
    _net.Commands=coNet.createCommands(_net);

    _net.Error=function(Code){
      switch (Code) {
        case coNet.CO_STATUS_FAIL                          : return "General fail code ["+ coNet.CO_STATUS_FAIL+"].";
        case coNet.CO_STATUS_OK                            : return "Success ["+ coNet.CO_STATUS_OK+"].";
        case coNet.CO_STATUS_WAIT                          : return "Waiting ["+ coNet.CO_STATUS_WAIT+"].";
        case coNet.CO_STATUS_REDIRECT                      : return "Redirect ["+ coNet.CO_STATUS_REDIRECT+"].";
        case coNet.CO_STATUS_AUTHENTICATE                  : return "Authenticate ["+ coNet.CO_STATUS_AUTHENTICATE+"].";

        case coNet.CO_STATUS_RESET                         : return "Network connect was reset ["+ coNet.CO_STATUS_RESET+"].";
        case coNet.CO_STATUS_ERROR                         : return "Task resulted in error ["+ coNet.CO_STATUS_ERROR+"].";
        case coNet.CO_STATUS_TIMEOUT                       : return "Time allocated for task expired ["+ coNet.CO_STATUS_TIMEOUT+"].";
        case coNet.CO_STATUS_EXCEPTION                     : return "Task resulted in exception ["+ coNet.CO_STATUS_EXCEPTION+"].";
        case coNet.CO_STATUS_ERR_CO_NOT_FOUND              : return "Core Object was not found ["+ coNet.CO_STATUS_ERR_CO_NOT_FOUND+"].";
        case coNet.CO_STATUS_ERR_CO_FOLDER_NOT_FOUND       : return "Core Object folder was not found ["+ coNet.CO_STATUS_ERR_CO_FOLDER_NOT_FOUND+"].";
        case coNet.CO_STATUS_ERR_CO_RESOURCE_NOT_FOUND     : return "Core Object resource was not found ["+ coNet.CO_STATUS_ERR_CO_RESOURCE_NOT_FOUND+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_NOT_FOUND          : return "Core Object resource was not found ["+ coNet.CO_STATUS_ERR_CO_RESOURCE_NOT_FOUND+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_INITIALIZATION     : return "Core Object Command was not initialized properly before execution ["+ coNet.CO_STATUS_ERR_CO_CMD_INITIALIZATION+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_FOLDER_NOT_FOUND   : return "Core Object Command folder was not found ["+ coNet.CO_STATUS_ERR_CO_CMD_FOLDER_NOT_FOUND+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_RESOURCE_NOT_FOUND : return "Core Object Command resource was not found ["+ coNet.CO_STATUS_ERR_CO_CMD_RESOURCE_NOT_FOUND+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_AUTH_REQD          : return "Core Object Command requires authentication with matching acl entries ["+ coNet.CO_STATUS_ERR_CO_CMD_AUTH_REQD+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_REDIRECT           : return "Core Object Command is redirecting output via another command ["+ coNet.CO_STATUS_ERR_CO_CMD_REDIRECT+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_DBMS_FAILURE       : return "Core Object Command resulted in a database error ["+ coNet.CO_STATUS_ERR_CO_CMD_DBMS_FAILURE+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_NOT_UNDERSTOOD     : return "Core Object Command was not understood ["+ coNet.CO_STATUS_ERR_CO_CMD_NOT_UNDERSTOOD+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_NOT_IMPLEMENTED    : return "Core Object Command is not implemented ["+ coNet.CO_STATUS_ERR_CO_CMD_NOT_IMPLEMENTED+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_NOT_INITIALIZED    : return "Core Object Command was not initialized ["+ coNet.CO_STATUS_ERR_CO_CMD_NOT_INITIALIZED+"].";
        case coNet.CO_STATUS_ERR_CO_CMD_ACCESS_DENIED      : return "Core Object Command access was denied ["+ coNet.CO_STATUS_ERR_CO_CMD_ACCESS_DENIED+"].";
      ...
      switch (Socket.readyState) {
        ...
        case 4: // request finished and response is ready
          if ((Socket.status==200) || (Socket.status==417)) {
            ...
            sData=sData.replace(/\r/g,coNet.CRLF);
            sData=sData.substring(0,sData.length-2);
            sData+=coNet.HEADER_SEPARATOR;
  ...
  createCommands:function(net){
    ...
    cmds.Headers=coList.kpList(coNet.FIELD_SEPARATOR,coNet.CRLF);
    cmds.Net=net;
    cmds.nextID=function(){
      var cmds=this;
      var res=coNet.nextID;
      coNet.nextID++;
      return res;
    };
    ...
    cmds.createCommand=function(net,sNSCore,sNSCommand,sData,onComplete,onError,onTimeOut,onProgress,Suspended,FreeOnComplete,AutoLoad) {
      ...
      cmd.ID=cmds.nextID();
      cmd.Headers=coList.kpList(coNet.FIELD_SEPARATOR,coNet.CRLF);
      cmd.Headers.Add(coNet.fieldID,cmd.ID.toString());
      cmd.Headers.Add(coNet.fieldNSCore,sNSCore);
      cmd.Headers.Add(coNet.fieldNSCommand,sNSCommand);
      ...
      cmd.toMessage=function(){
        ...
        sResult+=coNet.HEADER_SEPARATOR;
      ...
      cmd.doWSSend=function(){
        ...
        if (coNet.debugToConsole==true) coVDM.VDM.Console.Append(
          "coNet - WebSocket Send (".concat(
      ...
      cmd.doHTTPSend=function(){
        ...
        if (coNet.debugToConsole==true) coVDM.VDM.Console.Append("coNet - HTTP Send");
        cmd.Headers.Update(coNet.fieldNSCore,cmd.NSCore);
        cmd.Headers.Update(coNet.fieldNSCommand,cmd.NSCommand);
        cmd.Headers.Update(coNet.fieldAuth,cmd.Net.Creds.Auth);
        cmd.Headers.Update(coNet.fieldAccount,cmd.Net.Creds.User);
        ...
        for (var iLcv=0; iLcv<cmd.Headers.length; iLcv++){
          ...
          var iDX=coNet.RestrictedHeaders.IndexOf(itm.Name);
          if (iDX==-1)
      ...
      cmd.TimedOut=function(){
        ...
        var cmds=cmd.Commands;
        cmd.Expires=0;
        cmd.Code=coNet.CO_STATUS_TIMEOUT;
        cmd.Suspended=true;
        ...
        if (cmd.onTimeOut) {
          try {
            ...
            "coNet.Commands.Command.onTimeOut Exception".concat(
              " exception = ",err,
              " fieldNSCore=",cmd.NSCore,
              " fieldNSCommand=",cmd.NSCommand,
              " fieldID=",cmd.ID.toString(),
        ...
        if (cmd._onError) {
          try {
            ...
            if (coVDM.VDM.Console) {
              coVDM.VDM.Console.Append(
              "coNet.Commands.Command.onError Exception".concat(
                " exception = ",err,
                " fieldNSCore=",cmd.NSCore,
                " fieldNSCommand=",cmd.NSCommand,
                " fieldID=",cmd.ID.toString(),
      ...
      cmd.Send = (coNet.WebSockets==true) ? cmd.doWSSend : cmd.doHTTPSend ;
      ...
      cmd.createConnection=function(){
        ...
        con.Connect=function(){
          ...
          if (con.Trys<coNet.NET_TRIES) {
            con.Trys++;
            con.dtOpened=null;
            if (con.Net.WebSockets==true) {
              if (coNet.debugToConsole==true) coVDM.VDM.Console.Append("coNet Connection - Creating WebSocket");
              con.Socket=new WebSocket(con.Net.URI,coNet.Protocol);
              if (con.Socket) {
                con.Socket.onmessage=function(evt) {
                  if (coNet.debugToConsole==true) coVDM.VDM.Console.Append("coNet Connection - WebSocket Message Data");
                  var idx=evt.data.indexOf(coNet.HEADER_SEPARATOR);
                ...
                con.Socket.onopen=function(){
                  if (coNet.debugToConsole==true) coVDM.VDM.Console.Append("coNet Connection - WebSocket Opened");
                  con.Trys=0;
                  con.dtOpened=new Date();
                  con.Connected=true;
                };
                con.Socket.onclose=function(){
                  if (coNet.debugToConsole==true) coVDM.VDM.Console.Append("coNet Connection - WebSocket Closed");
                  con.Connected=false;
              ...
              con.Socket.open( (cmd.dataSend) ? "PUT" : "GET",sURL,coNet.NET_ASYNC);
            ...
            if (coNet.debugToConsole==true) coVDM.VDM.Console.Append("coNet Connection - Socket Failure");
            con.Net.Failed=true;
            con.Net.onNetworkFailure();
            return false;
          } else {
            if (coNet.debugToConsole==true) coVDM.VDM.Console.Append("coNet Connection - Socket Refailure");
            con.Net.onNetworkFailure();
            return false;
          };
        };
        con.Free=function(){
          ...
          if (con.Socket!=null) {
            ...
            con.Socket=null;
            con=con.Command.Connection=coObject.Release(con);
            return null;
          };
          con=coObject.Release(con);
          return null;
        };
        return con;
      };
    ...
    cmds.fromMessage=function(sHeaders,Data){
      ...
      var iID=cmds.Headers.getValueAsInt(coNet.fieldID);
      var iRemoteIP=cmds.Headers.getValueAsInt(coNet.fieldRemoteIP);
      cmd=cmds.Find(iID);
      if (cmd!=null)  {
        ...
        cmd.dataRecv=Data;
        cmd.Code=cmds.Headers.getValueAsInt(coNet.fieldCode,coNet.CO_STATUS_FAIL);
        cmd.onComplete(cmd);
        cmd.Headers.Clear();
        cmd.Try=0;
        if (coNet.debugToConsole==true) coVDM.VDM.Console.Append(
          "coNet - cmdQueue.fromMessage".concat(
            " fieldNSCore=",cmd.NSCore,
            " fieldNSCommand=",cmd.NSCommand,
            " fieldID=",cmd.ID.toString(),
            " Code=",cmd.Code,
    ...
    cmds.onWSTimer=function(tmr){
          ...
          case coNet.SOCKET_CLOSED :
            if (tmr.Net.Shutdown==true) {
              ...
              for (var iLcv=0; iLcv<tmr.Net.Queue.List.length; iLcv++){
                ...
                if (cmd.Error==false) {
                  cmd.Error=true;
                  cmd.Code=coNet.CO_STATUS_RESET;
                  cmd.onError(cmd);
                  if (cmd.FreeOnComplete==true)
                    cmd.Free();
                };
      ...
      /*
        ...
        if (tmr.Net.Failed==false) {
          ...
          dtExpires.setMilliseconds(coNet.NET_CON_PAUSE);
          if ( (tmr.Net.Connection.dtClosed==null) || (dtNow>dtExpires) ) {
            if (
                 (tmr.Net.Connection.dtOpened==null) &&
                 ( (tmr.Net.Connection.Socket==null)  || (tmr.Net.Connection.Socket!=null) && (tmr.Net.Connection.Socket.readyState!=coNet.SOCKET_OPENING))
            )
    ...
    cmds.Free=function(){
      var cmds=this;
      var idx=coNet.CommandsManifest.indexOf(cmds);
      if (idx!=-1) coNet.CommandsManifest.splice(idx,1);
      cmds.Timer.Free();
      cmds.Clear();
      cmds.Net=null;
    ...
    cmds.Timer=net.Timers.createTimer((coNet.WebSockets==true) ? function(tmr){cmds.onWSTimer(tmr);} : function(tmr){ cmds.onHTTPTimer(tmr);},coNet.msPolling,0);
    coNet.CommandsManifest.push(cmds);
    return cmds;
  },
  Free : function(net){
    net.Timers.Free();
  ...
  AutoLoad : function(){
    var lst=coNet.CommandsManifest;
