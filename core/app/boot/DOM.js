const DOM={
  ...
  enableSelect : function(el){
    ...
    for (var iLcv=0; iLcv<el.childNodes.length; iLcv++) {
      ...
      if (n.style) DOM.enableSelect(n);
    };
  },
  currentTarget  : function(e){
    if (e.currentTarget){
      DOM.currentTarget=function(e){return e.currentTarget;};
      return e.currentTarget;
    } else{
      DOM.currentTarget=function(e){return e.target;};
      return e.target;
    };
  },
  getChildNodeByTitle:function(srcElm,sTitle){
    ...
    for (var iLcv=0; iLcv<srcElm.childNodes.length; iLcv++){
      ...
      if (chNode.title==sTitle) {
        ...
        var res=DOM.getChildNodeByTitle(chNode,sTitle);
        if (res!=null)
          return res;
      };
    };
  ...
  PrintImage:function(sTitle,sImageData){
    ...
    prnW.onafterprint=DOM.CloseWindow;
  ...
  init : function(){
    ...
    s+="<script src=\"/core/app/DOM.js\" type=\"text/javascript\"></script>";
    s+="<title>$Title</title>";
    s+="</head>";
    s+="<body class=\"PrintWindow\" onafterprint=\"DOM.CloseWindow();\">";
    ...
    DOM.Browser=DOM.createBrowser();
    if (DOM.Browser.WebKit==true) {
    ...
    } else if (DOM.Browser.FireFox==true) {
      this.setBoxShadow=function(elm,sValue){
    ...
    if (textContent==true) {
      ...
      this.parseText=function(elm){
        var s="";
        switch (elm.nodeName) {
          case ("DIV")   : s=DOM.parseChildNodes(elm.childNodes); break;
          case ("P")     : s="\r\n\r\n" + DOM.parseChildNodes(elm.childNodes); break;
  ...
  getAllScripts: function(){
    ...
    for (var iLcv=0; iLcv<all.length; iLcv++){
      s=all[iLcv].attributes['scan'];
      if (s!=undefined){
        try {
          c = eval(s.nodeValue);
          c.Compiled=true;
        } catch (err) {
           c = Objects.createNew("Script");
           c.Unit=all[iLcv].attributes['src'].value;
           c.Compiled=false;
           AppKit.logConsoleMessage("Script (eval) : "+c.Unit+" "+err.message);
        }
  ...
  createConsoleLog: function(){
    let cl = Objects.createNew("Console");
    cl.Frame = document.createElement('div');
    document.body.appendChild(cl.Frame);
    cl.Frame.className=cl.Class;
    
    cl.Client=document.createElement('div');
    cl.Frame.appendChild(cl.Client);
    cl.Client.className=cl.Class+"Client";
    cl.writeLn=function(line){
      cl = this;
      cl.Frame.style.display="block";
      let s = document.createElement('span');
      cl.Client.appendChild(s);
      s.className=cl.Class+"Entry";
      DOM.setText(s,line);
    };
    return cl;
  },
  loadScript : function(owner,url,onLoadFunction,onErrorFunction){
    if(typeof(url) == 'string'){
      ...
      throw("DOM.loadScript requires a url parameter[0] ");
    }
  },
  loadStyle : function(owner,url,onLoadFunction,onErrorFunction){
    if(typeof(url) == 'string'){
      ...
      throw("DOM.loadStyle requires a url parameter[0] ");
    }
  },
  ...
  createBrowser : function(){
    var wb=Objects.createNew("Browser");
    ...
    if ( (wb.MSIE==true) || (wb.Trident==true)) {
      ...
      if ((iCheck<10) && (DOM.Redirect==true))
      ...
      var sOrganization = navigator.vendor;
        ...
        var sVersCopy     = "";
        var iCheck=coUtils.parseInt(sVersMajor);
        if ((iCheck<18) && (DOM.Redirect==true))
          window.location=wb.outdatedRedirect;
    } else {
        var saVersion=sVersion.split(" ");
        if (saVersion.length>0) sVersion=saVersion[0];
  ...
  ConsealImages:function(elm){
    for (var iLcv=0; iLcv<elm.childNodes.length; iLcv++){
      ...
      if (cElm.tagName==DOM.tagImage) {
        cElm.style.display="none";
      } else {
        DOM.ConsealImages(cElm);
      };
    };
  },
