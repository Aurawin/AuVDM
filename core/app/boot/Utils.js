const Utils= {
  ...
  xBias:function(itm){
    var marginLeft =(itm.style.marginLeft=='') ? Utils.parseInt(Utils.getStyle(itm,'margin-left')) : Utils.parseInt(itm.style.marginLeft);
    var marginRight = (itm.style.marginRight=='') ? Utils.parseInt(Utils.getStyle(itm,'margin-right')) : Utils.parseInt(itm.style.marginRight);
    var paddingLeft = (itm.style.paddingLeft=='') ? Utils.parseInt(Utils.getStyle(itm,'padding-left')) : Utils.parseInt(itm.style.paddingLeft);
    var paddingRight = (itm.style.paddingRight=='') ? Utils.parseInt(Utils.getStyle(itm,'padding-right')) : Utils.parseInt(itm.style.paddingRight);

    return paddingLeft + paddingRight + marginLeft + marginRight;
  },
  yBias:function(itm){
    var marginTop =(itm.style.marginTop=='') ? Utils.parseInt(Utils.getStyle(itm,'margin-top')) : Utils.parseInt(itm.style.marginTop);
    var marginBottom = (itm.style.marginBottom=='') ? Utils.parseInt(Utils.getStyle(itm,'margin-bottom')) : Utils.parseInt(itm.style.marginBottom);
    var paddingTop = (itm.style.paddingTop=='') ? Utils.parseInt(Utils.getStyle(itm,'padding-top')) : Utils.parseInt(itm.style.paddingTop);
    var paddingBottom = (itm.style.paddingBottom=='') ? Utils.parseInt(Utils.getStyle(itm,'padding-bottom')) : Utils.parseInt(itm.style.paddingBottom);
  ...
  parseInt : function(sVal,Def){
    ...
    for (var iLcv=0; iLcv<sVal.length; iLcv++){
      ...
      var idx=Utils.Digits.indexOf(chr);
      if (idx!=-1){
        sInt+=chr;
      } else {
        break;
